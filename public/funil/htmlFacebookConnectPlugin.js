...
var __fbCallbacks = [];
/* */
var APP_ID = "1730746240513468"
var htmlFacebookConnectPlugin = {}

htmlFacebookConnectPlugin.getLoginStatus = function getLoginStatus (s, f) {
  if (!__fbSdkReady) {
...
htmlFacebookConnectPlugin.graphApi = function (graphPath, s, f) {
...
htmlFacebookConnectPlugin.showDialog = function showDialog (options, s, f) {
  if (!__fbSdkReady) {
    return __fbCallbacks.push(function() {
...
htmlFacebookConnectPlugin.login = function login (permissions, s, f) {
  if (!__fbSdkReady) {
    return __fbCallbacks.push(function() {
...
htmlFacebookConnectPlugin.getAccessToken = function getAccessToken (s, f) {
...
htmlFacebookConnectPlugin.logEvent = function logEvent (eventName, params, valueToSum, s, f) {
  if (!__fbSdkReady) {
    return __fbCallbacks.push(function() {
...
htmlFacebookConnectPlugin.logPurchase = function logPurchase (value, currency, s, f) {
  if (!__fbSdkReady) {
    return __fbCallbacks.push(function() {
...
htmlFacebookConnectPlugin.appInvite = function appInvite (options, s, f) {
  // App Invites are not avaliable in JS.
  if(s) s()
}

htmlFacebookConnectPlugin.logout = function logout (s, f) {
  if (!__fbSdkReady) {
    return __fbCallbacks.push(function() {
      logout(s, f);
    });
...
htmlFacebookConnectPlugin.api = function api (graphPath, permissions, s, f) {
  if (!__fbSdkReady) {
    return __fbCallbacks.push(function() {
...
htmlFacebookConnectPlugin.browserInit = function browserInit (appId, version, s) {
